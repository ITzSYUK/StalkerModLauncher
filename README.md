...
Не переносите вручную весь workspace во время работы launcher. Каталог `current` является
восстанавливаемым кэшем: его безопаснее пересобрать на новом месте. Ценные данные профиля
находятся в `userdata`.

Сейчас launcher автоматически выбирает управляемый корень workspace на диске базовой игры.
Выбор произвольного корня workspace через интерфейс пока не поддерживается. Для резервного
копирования достаточно сохранить `userdata`; папку `current` копировать не требуется.
